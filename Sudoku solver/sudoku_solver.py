...
def boxes_array(n = 1):
    ...
    !!!
    Although this function produces an identical output to the hardcoded <boxes>,
    for some unexplained reason the number susbtitution does not work afterwards.
    As I'm unable to locate the bug, I'm forced to hardcode this.'''
        ...
        for j in range(9):
            for number in range(1,10):
                if number in poss[i][j]:
                    x = (n+1)*i   + 1 + offset[number][1]
                    y = (2*n+2)*j + 2 + offset[number][0]
                    boxes[x][y] = number + 48
